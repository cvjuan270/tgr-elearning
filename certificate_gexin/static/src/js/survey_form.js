odoo.define("survey.form", function (require) {
    ...
    publicWidget.registry.SurveyFormWidget = publicWidget.Widget.extend(SurveyPreloadImageMixin, {
        ...
        _onKeyDown: function (event) {
            ...
            } else if (self.options.questionsLayout === "page_per_question" && letter.match(/[a-z]/i)) {
        ...
        _onChangeChoiceItem: function (event) {
            ...
            if ($target.attr("type") === "radio") {
                ...
                var questionHasComment =
                    $target.closest(".o_survey_form_choice").find(".o_survey_comment").length !== 0 ||
                ...
                if ($matrixBtn.length > 0) {
                    ...
                    if (
                        ...
                        Object.keys(this.options.triggeredQuestionsByAnswer).includes($target.val())
                    ) {
        ...
        _onRadioChoiceClick: function (event) {
            ...
            if ($target.hasClass("o_survey_form_choice_item_selected")) {
                ...
                this.$(`input:radio[name="${$target.prop("name")}"].o_survey_form_choice_item_selected`).removeClass(
        ...
        _onNotification: function ({detail: notifications}) {
            ...
            if (notifications && notifications.length !== 0) {
                notifications.forEach(function (notification) {
                    if (notification.type === "next_question" || notification.type === "end_session") {
            ...
            if (nextPageEvent) {
                if (nextPageEvent.type === "next_question") {
                    var serverDelayMS =
                        moment.utc().valueOf() - moment.unix(nextPageEvent.payload.question_start).utc().valueOf();
                    if (serverDelayMS < 0) {
                        serverDelayMS = 0;
        ...
        _initDateTimePicker: function ($dateGroup) {
            ...
            var datetimepickerFormat =
                questionType === "datetime" ? time.getLangDatetimeFormat() : time.getLangDateFormat();
