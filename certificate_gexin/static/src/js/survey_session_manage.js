odoo.define("survey.session_manage", function (require) {
    ...
    publicWidget.registry.SurveySessionManage = publicWidget.Widget.extend(
        SurveyPreloadImageMixin,
        {
            selector: ".o_survey_session_manage",
            events: {
                "click .o_survey_session_copy": "_onCopySessionLink",
                "click .o_survey_session_navigation_next, .o_survey_session_start":
                    "_onNext",
            ...
            start: function () {
                ...
                return this._super.apply(this, arguments).then(function () {
                    ...
                    self.sessionShowLeaderboard = self.$el.data(
                        "sessionShowLeaderboard"
                    );
                    ...
                    // Copy link tooltip
                    self.$(".o_survey_session_copy").tooltip({
                        delay: 0,
            ...
            _onNext: function (ev) {
                ...
                if (screenToDisplay === "userInputs") {
                    ...
                    ["leaderboard", "leaderboardFinal"].includes(screenToDisplay) &&
                ...
                if (screenToDisplay === "question") {
                    ...
                    if (!this.resultsRefreshInterval) {
                        this.resultsRefreshInterval = setInterval(
                            this._refreshResults.bind(this),
                            2000
                        );
                    }
                } else if (screenToDisplay === "results") {
            ...
            _getNextScreen: function () {
                if (this.currentScreen === "question" && this.isScoredQuestion) {
                    ...
                    if (
                        ["question", "userInputs", "results"].includes(
                            this.currentScreen
                        ) &&
            ...
            _nextQuestion: function (goBack) {
                ...
                var nextQuestionPromise = this._rpc({
                    route: _.str.sprintf(
                        "/survey/session/next_question/%s",
                        self.surveyAccessToken
                    ),
                    params: {
                        go_back: goBack,
                    },
            ...
            _onNextQuestionDone: async function (goBack) {
                ...
                if (this.nextQuestion.question_html) {
                    ...
                    this._setupLeaderboard().then(function () {
                        self.$(".o_survey_session_leaderboard_title").text(
                            _t("Final Leaderboard")
                        );
                ...
                if (this.refreshBackground) {
                    $("div.o_survey_background").css(
                        "background-image",
                        "url(" + this.nextQuestion.background_image_url + ")"
                    );
            ...
            _startTimer: function () {
                ...
                if ($timer.length) {
                    ...
                    this.surveyTimerWidget =
                        new publicWidget.registry.SurveyTimerWidget(this, {
                            timer: timer,
                    ...
                    function (questionResults) {
                        if (questionResults) {
                            ...
                            if (
                                ...
                                self.resultsChart.updateChart(
                                    JSON.parse(
                                        questionResults.question_statistics_graph
                                    )
                                );
                            } else if (self.textAnswers) {
                                self.textAnswers.updateTextAnswers(
                                    questionResults.input_line_values
                                );
                            }
                            ...
                            if (self.attendeesCount && self.attendeesCount > 0) {
                                ...
                                ).text(`${answerCount} / ${self.attendeesCount}`);
                            }
                        }
            ...
            _setupChart: function () {
                ...
                if (!this.isStartScreen && this.showBarChart) {
                    ...
                    return this.resultsChart.attachTo(
                        this.$(".o_survey_session_chart")
                    );
                } else {
                    return Promise.resolve();
                }
            ...
            _setupLeaderboard: function () {
                ...
                if (this.isScoredQuestion || this.isLastQuestion) {
                    ...
                    return this.leaderBoard.attachTo(
                        this.$(".o_survey_session_leaderboard")
                    );
                } else {
                    return Promise.resolve();
                }
            ...
            _setupTextAnswers: function () {
                ...
                if (!this.isStartScreen && this.showTextAnswers) {
                    ...
                    return this.textAnswers.attachTo(
                        this.$(".o_survey_session_text_answers_container")
                    );
                } else {
                    return Promise.resolve();
                }
            ...
            _setupIntervals: function () {
                this.attendeesCount = this.$el.data("attendeesCount")
                    ? this.$el.data("attendeesCount")
                    : 0;

                if (this.isStartScreen) {
                    this.attendeesRefreshInterval = setInterval(
                        this._refreshAttendeesCount.bind(this),
                        2000
                    );
                } else {
            ...
            _setupCurrentScreen: function () {
                ...
                this.$(".o_survey_session_navigation_previous").toggleClass(
                    "d-none",
